...
-type nova_ws_state() :: #{controller_data := map(),
                           mod := atom(), subprotocols := [any()],
                           nova_handler := nova_ws_handler, _ := _}.
...
init(Req, State) ->
    ...
    {ok, PrePlugins} =
        nova_plugin:get_plugins(pre_ws_upgrade),
    case run_plugins(PrePlugins,
                     pre_ws_upgrade,
                     State#{req => Req})
        of
        {ok,
         State0 = #{controller_data := ControllerData,
                    mod := Mod}} ->
            ...
            case Mod:init(ControllerData0) of
                {ok, NewControllerData} ->
                    {cowboy_websocket,
                     Req,
                     State0#{controller_data => NewControllerData}};
                Error ->
                    ?ERROR("Websocket handler ~p returned unkown "
                           "result ~p",
                           [Mod, Error]),
...
websocket_init(State = #{mod := Mod}) ->
    case erlang:function_exported(Mod, websocket_init, 1) of
        true -> handle_ws(Mod, websocket_init, [], State);
        _ -> {ok, State}
    end.
...
handle_ws(Mod, Func, Args,
          State = #{controller_data := _ControllerData}) ->
    {ok, PrePlugins} =
        nova_plugin:get_plugins(pre_ws_request),
...
handle_ws(Mod, Func, Args, State) ->
    handle_ws(Mod,
              Func,
              Args,
              State#{controller_data => #{}}).

invoke_controller(Mod, Func, Args,
                  State = #{controller_data := ControllerData}) ->
    try ControllerResult = case erlang:apply(Mod,
                                             Func,
                                             Args ++ [ControllerData])
                               of
        ...
        case ControllerResult of
            ...
            {_, State0} ->
                {ok, PostPlugins} =
                    nova_plugin:get_plugins(post_ws_request),
                run_plugins(PostPlugins, post_ws_request, State0);
            _ -> ControllerResult
        end
    catch
        Type:Reasons:Stacktrace ->
            ?ERROR("Websocket failed with ~p:~p.~nStacktrace:~n~p",
                   [Type, Reasons, Stacktrace]),
            {stop, State}
    end.
