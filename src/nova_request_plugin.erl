...
pre_http_request(State, Options) ->
    Options0 = [K
                || {K, V} <- maps:to_list(Options), V == true],
...
plugin_info() ->
    {<<"Nova body plugin">>,
     ...
     <<"This plugin modulates the body of a "
       "request.">>,
     [{parse_bindings,
       <<"Used to parse bindings and put them "
         "in state under `bindings` key">>},
      {read_body,
       <<"Reads the body and put it under the "
         "`body`">>},
      {decode_json_body,
       <<"Decodes the body as JSON and puts it "
         "under `json`">>},
      {parse_qs,
       <<"Used to parse qs and put hem in state "
         "under `qs` key">>}]}.
...
modulate_state(State = #{req := Req,
    ...
    modulate_state(State#{controller_data =>
                              ControllerData#{bindings => Bindings}},
                   Tl);
modulate_state(State = #{req :=
                             Req = #{headers :=
                                         #{<<"content-type">> :=
    ...
    case cowboy_req:has_body(Req) of
        true ->
            ...
            modulate_state(State#{req => Req0,
                                  controller_data => ControllerData#{json => JSON}},
                           Tl);
        false ->
            modulate_state(State#{controller_data =>
                                      ControllerData#{json => #{}}},
...
modulate_state(State = #{req := Req,
    ...
    {ok, Data, Req0} = cowboy_req:read_body(Req),
    modulate_state(State#{req => Req0,
                          controller_data => ControllerData#{body => Data}},
                   Tl);
modulate_state(State = #{req := Req,
                         controller_data := ControllerData},
               [parse_qs | T1]) ->
