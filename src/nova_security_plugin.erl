...
pre_http_request(State = #{req := Req,
                           secure := {Module, Function}},
                 _Options) ->
    try Module:Function(Req) of
        {true, AuthData} ->
            {ok,
             State#{controller_data => #{auth_data => AuthData}}};
        true -> {ok, State};
        false ->
            {ok, State2} = nova_http_handler:render_page(401,
                                                         State),
            {stop, State2};
        {false, Headers} ->
            {ok, #{req := Req0} = State2} =
                nova_http_handler:render_page(401, State),
            Headers0 = cowboy_req:resp_headers(Req0),
            Req1 = cowboy_req:set_resp_headers(maps:merge(Headers0,
                                                          Headers),
                                               Req0),
            {stop, State2#{req => Req1}};
        {redirect, Route} ->
            Req0 = cowboy_req:set_resp_headers(#{<<"Location">> =>
                                                     list_to_binary(Route)},
                                               Req),
            {stop, State#{resp_status := 302, req := Req0}}
    catch
        Class:Reason ->
            ?ERROR("pre_http_request: ~p:~p failed with "
                   "~p/~p",
                   [Module, Function, Class, Reason]),
            {error, Reason}
    end;
pre_http_request(State, _Options) -> {ok, State}.
...
pre_ws_upgrade(State = #{req := Req,
    ...
    try Module:Function(Req) of
        ...
        {false, Headers} ->
            ...
            Req0 = cowboy_req:set_resp_headers(maps:merge(Headers0,
                                                          Headers),
                                               Req),
        ...
        Class:Reason ->
            ?ERROR("pre_ws_upgrade: ~p:~p failed with ~p/~p",
                   [Module, Function, Class, Reason]),
            {error, Reason}
    end;
pre_ws_upgrade(State, _Options) -> {ok, State}.
...
post_http_request(State, _Options) -> {ok, State}.
...
plugin_info() ->
    {<<"Nova Security Plugin">>,
     ...
     <<"Plugin that handles the security around "
       "paths.">>,
     []}.
