...
-callback init(State :: map()) -> {ok,
                                   State0 :: map()} |
                                  any().

-callback websocket_init(State :: map()) -> Result ::
                                                call_result().

-optional_callbacks([websocket_init/1]).

-callback websocket_handle(Frame :: in_frame(),
                           State :: map()) -> call_result().

-callback websocket_info(Info :: any(),
                         State :: map()) -> call_result().

-callback terminate(Reason :: reason(),
                    PartialReq :: map(), State :: map()) -> ok.
