...
init(StreamID, Req, Opts) ->
    Req0 = case nova:get_env(use_sessions, true) of
               true ->
    ...
    {Commands, Next} = cowboy_stream:init(StreamID,
                                          Req0,
                                          Opts),
...
data(StreamID, IsFin, Data,
    ...
    {Commands, Next0} = cowboy_stream:data(StreamID,
                                           IsFin,
                                           Data,
                                           Next),
...
info(StreamID, {response, Code, _Headers, _Body} = Info,
     State = #state{next = Next, req = Req})
    ...
    case nova_router:status_page(Code, Req) of
        {ok, StatusCode, StatusHeaders, StatusBody, _} ->
            {[{error_response,
             ...
             State};
        _ ->
            {Commands, Next0} = cowboy_stream:info(StreamID,
                                                   Info,
                                                   Next),
            {Commands, State#state{next = Next0}}
    end;
...
early_error(StreamID, Reason, PartialReq,
            {_, Status, _Headers, _} = Resp, Opts) ->
